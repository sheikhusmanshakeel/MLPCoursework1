...
cost = CECost()
model = MLP(cost=cost)
sigmoidLayer = Sigmoid(inputDimensions=784, outputDimensions=250, rng=rng)
sigmoidLayer2 = Sigmoid(inputDimensions=250, outputDimensions=250, rng=rng)
softmaxLayer = Softmax(inputDimensions=250, outputDimensions=10, rng=rng)
...
plt.title("Training Error - Hidden Layers : 2")
plt.show()
plt.plot(intermediateArrayValidation,  label='LR = {0} \n Hidden Units = 500')
plt.title("Validation Error - Hidden Layers : 2")
plt.show()
