...
cost = CECost()
model = MLP(cost=cost)
sigmoidLayer1 = Sigmoid(inputDimensions=784, outputDimensions=125, rng=rng)
sigmoidLayer2 = Sigmoid(inputDimensions=125, outputDimensions=125, rng=rng)
sigmoidLayer3 = Sigmoid(inputDimensions=125, outputDimensions=125, rng=rng)
sigmoidLayer4 = Sigmoid(inputDimensions=125, outputDimensions=125, rng=rng)
softmaxLayer = Softmax(inputDimensions=125, outputDimensions=10, rng=rng)
...
plt.title("Training Error - Hidden Layers : 4")
plt.show()
plt.plot(intermediateArrayValidation,  label='LR = {0} \n Hidden Units = 500')
plt.title("Validation Error - Hidden Layers : 4")
plt.show()
